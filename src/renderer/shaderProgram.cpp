...
namespace GLTest {
    ...
    struct hashShaderVar {
        std::size_t operator()(const ShaderVariable& key) const {
            // This code was adapted from https://stackoverflow.com/questions/35985960/c-why-boosthash-combine-the-best-way-to-combine-hash-values
            std::size_t seed = std::hash<std::string>()(key.name);
            seed ^= std::hash<u32>()(key.shaderProgramId) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
            return seed;
        }
    };
    ...
    void ShaderProgram::uploadUInt(const char* varName, u32 value) const {
        int varLocation = getVariableLocation(*this, varName);
    ...
    static GLint getVariableLocation(const ShaderProgram& shader, const char* varName) {
        ShaderVariable match = {varName, 0, shader.programId};
