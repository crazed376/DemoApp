...
float mapClouds(vec3 p) {
	float f = 2.5*fbm(1.2*p+vec3(.5,.9,1)*.1*u_time);
    f = mix(f,f*.1,.4+.6*noise(.5*p+vec3(.5,.3,1)*.05*u_time));
    return 1.5*f - 2. - p.y;
}
...
vec4 renderClouds(vec3 bgCol, float depth, vec3 ro, vec3 rd) {
    ...
    // dithering
    t += .2*hash(u_time+gl_FragCoord.x*8315.9213/u_resolution.x+gl_FragCoord.y*2942.5192/u_resolution.y);
...
void main() {
    ...
    for (int n=0; n<AA; n++) {
        ...
        vec2 p = (fragCoord+off - .5*u_resolution.xy) / u_resolution.y;
        
        vec3 ro = vec3(.2+.2*sin(.4*u_time),.7,4); // ray origin
